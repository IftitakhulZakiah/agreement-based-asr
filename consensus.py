...
# e.g. python consensus.py --cnn-ali cnn_ali --lstm-ali lstm_ali tri_ali dnn_ali
# python consensus.py --cnn-ali cnn/ali.all.txt --lstm-ali lstm/ali.all.txt triphone/ali.all.txt dnn/ali.all.txt
...
def get_args():
    ...
    parser.add_argument("--cnn-ali", type=str, help="CNN alignment directory")
    parser.add_argument("--lstm-ali", type=str, help="LSTM alignment directory")
    parser.add_argument("tri_ali", type=str, metavar="<tri-ali>")
...
def rearrangement(models_phone, models):
	""" Rearrangement the models_phone
	 before : {'tri' : {utt1 : [tri_frame1, tri_frame2, ..], utt2 : ..., ...}, 
				'dnn' : {utt1 : [dnn_frame1, dnn_frame2, ..], utt2 : ..., ...}, ...}
	 # the utts_phone
	 after : {utt1 : {'frame1' : [tri_frame1, dnn_frame1, cnn_frame1, lstm_frame1],
						 'frame2' : [tri_frame2, dnn_frame2, cnn_frame2, lstm_frame2],
	 					...}...} """
	...
	tri_model_phone = model_keys[0] # tri model as reference
	dnn_model_phone = model_keys[1]
	
	utts_phone = {}
	for curr_utt in list(models_phone[tri_model_phone].keys()): 
		...
		if is_exist:
			n_frame_tri = len(models_phone[tri_model_phone][curr_utt])
			...
			if len(model_keys) >= 2:
				is_has_same_n_frame = 1 if n_frame_tri == n_frame_dnn else 0
			if len(model_keys) >= 3:
				is_has_same_n_frame = 1 if is_has_same_n_frame and (n_frame_tri == n_frame_cnn) else 0
			if len(model_keys) == 4:
				is_has_same_n_frame = 1 if is_has_same_n_frame and (n_frame_tri == n_frame_lstm) else 0

			if is_has_same_n_frame:
				utts_temp = {}
				# utts_temp = []
				counter_frame = 0
				for j in range(len(models_phone[tri_model_phone][curr_utt])): # total frame in curr_utt
					frame_temp = []
						# try:
...
if __name__ == '__main__':
	...
	# 	tri_phones = args.tri_ali + '/phone_ali/ali.' + i + '.txt'

	# urutan models harus tri, dnn, cnn, lstm
	models = [args.tri_ali, args.dnn_ali, args.cnn_ali, args.lstm_ali]	
	...
	write_file(utts_frame_phones, 'phone_vote.txt')
	write_file(utts_frame_count, 'count_vote.txt')